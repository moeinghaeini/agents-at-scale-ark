...
`Evaluation` and `Evaluator` CRDs, controllers, webhooks, API endpoints, CLI commands, and dashboard pages have been removed from Ark core. Evaluations are now maintained as a standalone module in the [Ark Marketplace](https://github.com/mckinsey/agents-at-scale-marketplace). See [marketplace issue #129](https://github.com/mckinsey/agents-at-scale-marketplace/issues/129) for migration status.

**If you are using evaluations**, install the evaluator module from the marketplace after upgrading. Existing `Evaluation` and `Evaluator` resources in your cluster will remain but are no longer reconciled until the marketplace module is installed. To clean up orphaned resources instead:

```bash
kubectl delete evaluations --all -A
kubectl delete evaluators --all -A
kubectl delete crd evaluations.ark.mckinsey.com evaluators.ark.mckinsey.com
```

**If you are not using evaluations**, no action is needed.
