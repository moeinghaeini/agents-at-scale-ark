...
2. If the target agent has an `executionEngine` (A2A or named engine), the controller executes it directly — bypassing the sidecar since these agents proxy to external services
3. Otherwise, sends an A2A `SendMessage` to the query engine sidecar with the target and query reference
4. Waits for the response and writes the result to the Query status

### 3. Query Engine Execution

The query engine receives the A2A message and:
