...
Use the `conversationId` field for multi-turn conversations, and `queryAnnotations` for custom Kubernetes annotations:
...
The `conversationId` is sent to the execution engine as A2A `contextId` for conversation threading. The `queryAnnotations` field can pass additional Kubernetes annotations to the Query resource. This format works with standard OpenAI SDKs since metadata fields accept string values.
...
Responses include an `ark` field with query metadata including status, token usage, and conversation ID:
...
<Tabs items={['Non-streaming', 'Streaming']}>
  <Tabs.Tab>
    ...
    # Access conversation ID for multi-turn conversations
    conversation_id = response.ark['completedQuery']['status']['conversationId']
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```python
    ...
    for chunk in stream:
        ...
        if hasattr(chunk, 'ark') and chunk.ark.get('completedQuery'):
            conversation_id = chunk.ark['completedQuery']['status']['conversationId']
    ```
  </Tabs.Tab>
</Tabs>
